...
public class gapPipe extends DynamicComponent {
    ...
    float pipeWidth = this.getWidth();
    float pipeHeight = this.getHeight();



       top_pipe = new Pipe(null, null, getColor());
       bottom_pipe = new Pipe(null, null, getColor());
        ...
        if(coinflip == 0){
          top_pipe_pos.setY(1.0f - top_pipe.getYTopLeft());
          bot_pipe_pos.setY(-1.0f + bot_pipe_dim.getHeight());
        }else{
          bot_pipe_pos.setY(1.0f - bottom_pipe.getYTopLeft());
